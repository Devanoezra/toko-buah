...
class M_Transaksi extends CI_Model
{
	...
	public function cekstok()
	{
		...
		for ($i = 0; $i < count($this->cart->contents()); $i++) {
			...
			$stok = $this->db->where('kode_buah', $this->input->post('kode_buah')[$i])->get('buah')->row()->stok;
	...
	public function simpan_db()
	{
		...
		for ($i = 0; $i < count($this->cart->contents()); $i++) {
			...
			$this->db->where('kode_buah', $this->input->post('kode_buah')[$i])->update('buah', $u_stok);
		}
		...
		for ($i = 0; $i < count($this->cart->contents()); $i++) {
			...
			$s_dttransaski[] = array(
				'kode_transaksi' => $ts->kode_transaksi,
				'kode_buah' => $this->input->post('kode_buah')[$i],
	...
	public function ambil_dts($kode_transaksi)
	{
		...
		return $this->db->join('buah', 'buah.kode_buah = detail_transaksi.kode_buah')
